...
export function LiveOutputViewer({
  ...
  const parseOutputLine = (line: string, index: number): ParsedLine | null => {
    ...
    const levelMatch = trimmedLine.match(
      /^\[?(INFO|WARN|ERROR|DEBUG|TRACE)\]?\s*:?\s*(.*)/i
    );
    if (levelMatch) {
    ...
    const timestampMatch = trimmedLine.match(
      /^(\d{4}-\d{2}-\d{2}[\sT]\d{2}:\d{2}:\d{2}(?:\.\d{3})?Z?)\s+(.*)/
    );
    if (timestampMatch) {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 relative">
        <div
          ...
          style={{
            scrollBehavior: job?.status === "running" ? "smooth" : "auto",
          }}
        >
            ...
            {filteredLines.length === 0 ? (
              ...
              <div className="space-y-1">
                {filteredLines.map((line, index) => (
                  <div
                    key={line.id}
                    className="whitespace-pre-wrap break-words hover:bg-gray-800/50 px-1 rounded"
                  >
                ...
                {job?.status === "running" &&
                  ...
                  levelFilter === "all" && (
                    <div className="flex items-center gap-1 mt-4 px-1 bg-gray-800/50 rounded p-2 border-l-2 border-green-400">
                      <span className="text-gray-500 mr-3 select-none">
                        {String(parsedOutput.totalLines + 1).padStart(4, " ")}:
                      </span>
                      <span className="text-green-300 animate-pulse">█</span>
                      <span className="text-gray-400 text-xs ml-2">
                        Executing •{" "}
                        {parsedOutput.lastUpdate.toLocaleTimeString()}
                      </span>
                      {isConnected && (
      ...
      <div className="p-3 border-t bg-gray-50 text-xs text-gray-600">
        <div className="flex items-center justify-between">
          <div>
            Total: {parsedOutput.totalLines} lines • Displayed:{" "}
            {filteredLines.length} lines • Status: {job?.status} •
            {job?.duration &&
              `Duration: ${Math.round(job.duration / 1000000000)}s`}
            {job?.status === "running" && (
          ...
          <div className="flex items-center gap-4">
            ...
            {job?.exit_code !== null && (
              <span>
                ...
                <span
                  className={
                    job.exit_code === 0 ? "text-green-600" : "text-red-600"
                  }
                >
                  {job.exit_code}
                </span>
