  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:scale-105 shadow-sm">
      <div className="flex items-center gap-3">
        <div className={`rounded-lg ${bgColor} p-2 transition-transform duration-200 hover:scale-110 ${isAnimating ? 'animate-pulse' : ''}`}>
          <Icon className={`h-5 w-5 ${color} transition-all duration-300`} />
        </div>
        <div>
          <p className={`text-2xl font-bold transition-all duration-300 ${isAnimating ? 'text-primary' : ''}`}>
            {animatedValue}
          </p>
